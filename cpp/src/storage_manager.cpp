...
std::atomic<int> StorageManager::disk_reads{0};
std::atomic<int> StorageManager::disk_writes{0};
...
void StorageManager::open(const std::string& file_path) {
    ...
    if (!std::filesystem::exists(file_path_)) {
        ...
        if (!create_file) { throw std::runtime_error("Failed to create database file: " + file_path_); }
    }
    file_.open(file_path_, std::ios::in | std::ios::out | std::ios::binary);
    if (!file_.is_open()) { throw std::runtime_error("Failed to open database file: " + file_path_); }
...
std::uint64_t StorageManager::pageCount() const { return num_pages_; }
...
void StorageManager::writePage(std::uint64_t page_id, const std::vector<std::uint8_t>& page_data) {
    std::lock_guard<std::mutex> lock(disk_latch_);
    ensureOpen();
    if (page_data.size() != kPageSize) { throw std::invalid_argument("writePage expects exactly 4096 bytes"); }
...
std::uint64_t StorageManager::allocatePage() {
    ...
    const auto offset = static_cast<std::streamoff>(new_page_id * kPageSize);
    file_.seekp(offset);
    file_.write(reinterpret_cast<const char*>(empty_page.data()), static_cast<std::streamsize>(kPageSize));
    if (!file_) { throw std::runtime_error("Failed to write page"); }
    ++num_pages_;
    return new_page_id;
}
...
void StorageManager::ensureOpen() const {
    if (!is_open_) { throw std::runtime_error("StorageManager file is not open"); }
}
